...
<div class="page-hero">
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h2><i class="bi bi-tags-fill me-2"></i>Jenis Buku</h2>
            <p>Kelola Jenis Buku untuk mengorganisir koleksi buku digital</p>
        </div>
        <button class="btn-add" data-bs-toggle="modal" data-bs-target="#modalTambah">
            <i class="bi bi-plus-circle-fill"></i> Tambah Jenis Buku
        </button>
    </div>
</div>
...
<div class="books-card">
    <div class="books-card-header">
        <h6>
            ...
            Semua Jenis Buku
            <span class="badge bg-secondary" style="font-size:11px;font-weight:600;">{{ $types->total() }}</span>
        </h6>
        <div class="search-wrap">
            <i class="bi bi-search"></i>
            <input type="text" id="searchInput" placeholder="Cari jenis buku...">
        </div>
    </div>

    <div class="table-responsive">
        <table class="table mb-0" id="typeTable">
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span style="font-weight:600;color:#1e293b;">{{ $type->name }}</span></td>
                    <td><code style="font-size:12px;color:#64748b;background:#f8fafc;padding:2px 8px;border-radius:5px;">{{ $type->slug }}</code></td>
                    <td>
                    ...
                    <td>
                        <div class="action-btns">
                            <button class="btn-action btn-edit" onclick="openEdit({{ $type->id }}, '{{ addslashes($type->name) }}')">
                                <i class="bi bi-pencil-fill"></i>
                            </button>
                            <button class="btn-action btn-delete" onclick="openDelete({{ $type->id }}, '{{ addslashes($type->name) }}', {{ $type->books_count ?? 0 }})">
                ...
                @empty
                <tr>
                    <td colspan="5" class="text-center py-5 text-muted">Belum ada data jenis buku.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div class="modal fade" id="modalTambah" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width:420px">
        <div class="modal-content">
            <form action="{{ route('admin.type-book.store') }}" method="POST">
                @csrf
                ...
                <div class="modal-body">
                    <label class="form-label-m">Nama Jenis Buku <span class="text-danger">*</span></label>
                    <input type="text" name="name" class="form-input-m" placeholder="Contoh: Cerita Rakyat" required>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width:420px">
        <div class="modal-content">
            <form id="formEdit" method="POST">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-light btn-sm" data-bs-dismiss="modal">Batal</button>
                    <button type="submit" class="btn-modal-save" style="background:#3b82f6">Perbarui</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width:400px">
        <div class="modal-content">
            <form id="formHapus" method="POST">
                ...
                @method('DELETE')
                <div class="modal-header">
                    <h5 class="modal-title text-danger">Hapus Jenis Buku</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body text-center">
                    <p>Hapus jenis buku "<span id="deleteNameDisplay" class="fw-bold"></span>"?</p>
                    <p id="deleteWarning" class="small text-muted"></p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-light btn-sm" data-bs-dismiss="modal">Batal</button>
                    <button type="submit" class="btn-modal-save" style="background:#ef4444">Ya, Hapus</button>
                </div>
            </form>
        </div>
    </div>
...
function openEdit(id, name) {
    ...
    document.getElementById('formEdit').action = '/sembari/admin/type-book/' + id;
    new bootstrap.Modal(document.getElementById('modalEdit')).show();
}

function openDelete(id, name, bookCount) {
    document.getElementById('deleteNameDisplay').textContent = name;
    document.getElementById('formHapus').action = '/sembari/admin/type-book/' + id;
    document.getElementById('deleteWarning').textContent = bookCount > 0 ? bookCount + ' buku terhubung ke sini.' : '';
